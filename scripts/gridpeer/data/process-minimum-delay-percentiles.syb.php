...
//echo $line;
	if (preg_match('$^([^:]+):.*\|\d+ DELAY AT ([0-9]+) IS ([0-9]+)$',$line,$m)) {
		...
		$lastdelay = $m[2]/$grainhold;
	
		//echo $lastpeer."  ".$lastdelay;
		//echo "\n";
		//$class = $m[1][0];
...
//if ($counttt++==100) break;
	} else if(preg_match('$^([^:]+):.*\|MINIMUM PLAYBACK LAG ([0-9]+)$',$line,$m)) {
		$parts = explode(":",$m[0]);
		$thispeer = $parts[0];
		
		if($thispeer == $lastpeer && $lastdelay != 0) {
			$class = $m[1][0];
	
			$minlag = round($m[2]/$grainlag);
			$hold = $lastdelay;
			$holdclass[$class[0]][$hold][] = $minlag;
			$holdclass['zall'][$hold][] = $minlag;
		}
	}
...
echo "percentile\tht\tlag-SY\tlag-LU\tlag-HU\tlag-all\n";
