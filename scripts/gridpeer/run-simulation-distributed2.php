...
for ($run=1; $run<=$runs; $run++) {
	...
	foreach (array_keys($usedhosts) as $host) {
		...
		$commands[$host][]="egrep -H 'LAG|called|DELAY|achieved|((sent|received|dht)MessagesAverage)|sendMessage\|sending message|reply\|sending reply|dhtOperation |in putSegmentBlock|duplicate block received|Playback failed|tuning' /tmp/LiveShift/log/liveshift.log.* | php -f /home/".$hostuser[$host]."/liveshift/scripts/gridpeer/data/delete-remainings.php $length > /tmp/LiveShift/log/pass2";
